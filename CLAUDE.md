...
สถานะล่าสุด (2026-07-22): การ์ดที่มี effect text 6,264 ใบ มี automatic handling แล้ว **~73.9%** (วัดด้วย `node tools/coverage-total.mjs` — มี ranking series ที่แย่สุดให้เลือกทำต่อ; ใช้ `node tools/coverage-report-full.mjs` เพื่อดูตารางทั้ง 55 series + "bonus text ค้าง" ของ Raid boss ด้วย)
- MCR: 137/178 · EVA: ~65/108 · **HTR: 175/179 (98%)** · **ARK: 168/174 (97%)** · **CGH: 168/169 (99%)** · **AND: 69/72 (96%)** · **SLG: 74/75 (99%)** · **JJK: 176/179 (98%)** · **BLC: 171/173 (99%)** · **TSK: 163/168 (97%)** · **KMY: 174/184 (95%)**
- **งานเรียงตาม series ที่แย่สุดก่อน** (ผู้ใช้สั่ง "/goal" ให้ทำต่อเนื่องจนกว่าจะจบ) — ทำ HTR→ARK→CGH→AND→SLG→JJK→BLC→TSK→KMY แล้ว ถัดไป: **KGR (59%)** ตามด้วย SMD/GMR/BLK — ดูจาก `coverage-total.mjs` ranking สดทุกครั้ง (อย่าพึ่งลิสต์เก่าใน CLAUDE.md เพราะเปลี่ยนทุกรอบ)
...
- **engine hook ใหม่ที่เพิ่มรอบนี้ (SLG pass)**: `onAnyAttack(atkUnit,selfUnit)` hook ใหม่ (การ์ด/Field ที่ตอบสนองเมื่อ "unit อื่นในสนามของคุณ" โจมตี ไม่ใช่แค่ตัวเอง — ใช้กับ effect แบบ "when one of your X attacks, you may rest this to..."), `unit.tempUntargetable` (คู่กับ `kw.untargetable` แบบชั่วคราว), `unit._grantedAttackDraw` (แจก "[When Attacking] draw" ชั่วคราว), แก้ retire-conditional matcher ให้รองรับ "sideline it"/"Outside Area it" เป็นกริยาแทน "retire it" (สไตล์ SLG)
- **engine hook รอบ AND**: `unit.effectsNullified` ("loses all of its effects"), `kw.unblockableBPMin` (static "cannot be blocked by BP N or more", พบ 92 ใบทั้งฐานข้อมูล), `onDefenderWinBattle` hook (การ์ดที่ block แล้วชนะ), `H.unraidTopLayer` (ลอกชั้นบนของ Raid state stack)
- **engine hook รอบ CGH**: `p._playedApCostsThisTurn`, `p._eventsUsedThisTurn`, `unit.enteredTurn`, `tempUnblockableBP`/`tempUnblockableBPMin`, `findSeg()` helper (เชื่อม on-event เข้ากับ on-play matcher เดิม — ผลกระทบสูงสุดในบรรดา fix ทั้งหมด)
...
| `js/effects/mcr.js`, `eva.js`, `htr.js`, `ark.js`, `cgh.js`, `and.js`, `slg.js`, `jjk.js`, `blc.js`, `tsk.js`, `kmy.js` | script รายใบ (registry per cardNo — override generic เสมอ) |
...
Unit state: `bpMod` (หมดเทิร์น) `bpPersist` (ถึงต้นเทิร์นหน้า) `tempImpact tempDmg tempGen tempFrontGen frontGenPersist counters retireAtEndOfMain retireAtEndOfTurn noBlock skipNextStand noRetire tempSnipe tempUnblockableBP tempUnblockableBPMin tempUntargetable effectsNullified enteredTurn _grantedOnWinDraw _grantedAttackDraw _playedByEffect _watchers[] _movedThisTurn`
Engine helpers: `sidelineUnit returnUnitToHand moveUnitFree playCardFromZone checkBpZero draw log payAP hasTextCostDiscount opponentOf bp parseKeywords activeAP hasEnergyFor scheduleDelayedAction`
Keyword flags (จาก `parseKeywords`, ถาวรไม่รีเซ็ตรายเทิร์น): `step snipe doubleAttack doubleBlock nullifyImpact impact dmg raidTargets entersActive entersActiveIf unblockableBP unblockableBPMin alsoTreatedAs frontGen untargetable cannotBlock cannotAttack`
