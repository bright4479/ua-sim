...
สถานะล่าสุด (2026-07-20): การ์ดที่มี effect text 6,264 ใบ มี automatic handling แล้ว **~67.2%** (วัดด้วย `node tools/coverage-total.mjs` — มี ranking series ที่แย่สุดให้เลือกทำต่อ; ใช้ `node tools/coverage-report-full.mjs` เพื่อดูตารางทั้ง 55 series + "bonus text ค้าง" ของ Raid boss ด้วย)
- MCR: 137/178 · EVA: ~65/108 · **HTR: 175/179 (98%)** · **ARK: 168/174 (97%)** · **CGH: 168/169 (99%, เหลือ 1 ใบ skip — field-wide targeting-protection aura ยังไม่มี engine mechanism)**
- **งานเรียงตาม series ที่แย่สุดก่อน** (ผู้ใช้สั่ง "/goal" ให้ทำต่อเนื่องจนกว่าจะจบ) — ทำ HTR→ARK→CGH แล้ว ลำดับถัดไปดูจาก `coverage-total.mjs` ranking สดทุกครั้ง (อย่าพึ่งลิสต์เก่าใน CLAUDE.md เพราะเปลี่ยนทุกรอบ)
...
- **engine hook ใหม่ที่เพิ่มรอบนี้ (CGH pass)**: `p._playedApCostsThisTurn` (Set ของ AP cost การ์ดที่เล่นจากมือเทิร์นนี้), `p._eventsUsedThisTurn` (นับจำนวน Event ที่ใช้เทิร์นนี้), `unit.enteredTurn` (`G.turn` ตอนที่ยูนิตนี้ลงสนาม จากทุกช่องทาง — เช็ค "การ์ดที่ลงสนามเทิร์นนี้"), `tempUnblockableBP`/`tempUnblockableBPMin` (แจก "ห้าม block ด้วย BP มาก/น้อยกว่า N" ชั่วคราว), `findSeg()` helper ใน common.js (ลอง matcher กับทุก '@'-segment — ใช้เชื่อม on-event เข้ากับ on-play matcher เดิม)
...
| `js/effects/mcr.js`, `eva.js`, `htr.js`, `ark.js`, `cgh.js` | script รายใบ (registry per cardNo — override generic เสมอ) |
