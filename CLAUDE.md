...
สถานะล่าสุด (2026-07-22): การ์ดที่มี effect text 6,264 ใบ มี automatic handling แล้ว **~78.0%** (วัดด้วย `node tools/coverage-total.mjs` — มี ranking series ที่แย่สุดให้เลือกทำต่อ; ใช้ `node tools/coverage-report-full.mjs` เพื่อดูตารางทั้ง 55 series + "bonus text ค้าง" ของ Raid boss ด้วย)
- MCR: 137/178 · EVA: ~65/108 · **HTR: 175/179 (98%)** · **ARK: 168/174 (97%)** · **CGH: 168/169 (99%)** · **AND: 69/72 (96%)** · **SLG: 74/75 (99%)** · **JJK: 176/179 (98%)** · **BLC: 171/173 (99%)** · **TSK: 163/168 (97%)** · **KMY: 174/184 (95%)** · **KGR: 74/78 (95%)** · **SMD: 70/77 (91%)** · **GMR: 73/75 (97%)** · **BLK: 112/113 (99%)** · **KMR: 187/202 (93%)** · **OPM: 104/108 (96%)** · **KIN: 71/73 (97%)**
- **งานเรียงตาม series ที่แย่สุดก่อน** (ผู้ใช้สั่ง "/goal" ให้ทำต่อเนื่องจนกว่าจะจบ) — ทำ HTR→ARK→CGH→AND→SLG→JJK→BLC→TSK→KMY→KGR→SMD→GMR→BLK→KMR→OPM→KIN แล้ว ถัดไป: **YYH (62%)** ตามด้วย HIQ/TLR/GIM — ดูจาก `coverage-total.mjs` ranking สดทุกครั้ง (อย่าพึ่งลิสต์เก่าใน CLAUDE.md เพราะเปลี่ยนทุกรอบ)
...
| `js/effects/mcr.js`, `eva.js`, `htr.js`, `ark.js`, `cgh.js`, `and.js`, `slg.js`, `jjk.js`, `blc.js`, `tsk.js`, `kmy.js`, `kgr.js`, `smd.js`, `gmr.js`, `blk.js`, `kmr.js`, `opm.js`, `kin.js` | script รายใบ (registry per cardNo — override generic เสมอ) |
...
Unit state: `bpMod` (หมดเทิร์น) `bpPersist` (ถึงต้นเทิร์นหน้า) `tempImpact tempDmg tempGen tempFrontGen frontGenPersist counters retireAtEndOfMain retireAtEndOfTurn noBlock skipNextStand noRetire tempSnipe tempUnblockableBP tempUnblockableBPMin tempUntargetable effectsNullified enteredTurn _grantedOnWinDraw _grantedAttackDraw _playedByEffect _watchers[] _movedThisTurn tempRaidable tempCannotMove`
