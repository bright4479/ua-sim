...
สถานะล่าสุด (2026-07-20): การ์ดที่มี effect text 6,264 ใบ มี automatic handling แล้ว **~68.2%** (วัดด้วย `node tools/coverage-total.mjs` — มี ranking series ที่แย่สุดให้เลือกทำต่อ; ใช้ `node tools/coverage-report-full.mjs` เพื่อดูตารางทั้ง 55 series + "bonus text ค้าง" ของ Raid boss ด้วย)
- MCR: 137/178 · EVA: ~65/108 · **HTR: 175/179 (98%)** · **ARK: 168/174 (97%)** · **CGH: 168/169 (99%)** · **AND: 69/72 (96%, เหลือ 3 ใบ skip — ดู comment ท้าย and.js)**
- **งานเรียงตาม series ที่แย่สุดก่อน** (ผู้ใช้สั่ง "/goal" ให้ทำต่อเนื่องจนกว่าจะจบ) — ทำ HTR→ARK→CGH→AND แล้ว ลำดับถัดไปดูจาก `coverage-total.mjs` ranking สดทุกครั้ง (อย่าพึ่งลิสต์เก่าใน CLAUDE.md เพราะเปลี่ยนทุกรอบ)
...
- **engine hook ใหม่ที่เพิ่มรอบนี้ (AND pass)**: `unit.effectsNullified` ("loses all of its effects" — ปิด onPlay/onAttack/onMain/bpBonus/genMod ของยูนิตนั้นทั้งหมด), `kw.unblockableBPMin` (static "cannot be blocked by BP N or more", พบ 92 ใบทั้งฐานข้อมูล!), `onDefenderWinBattle` hook ใหม่ (การ์ดที่ตั้ง block แล้วชนะ — ก่อนหน้านี้ไม่มี hook แยกจาก attacker win เลย), `H.unraidTopLayer` (ลอกชั้นบนของ Raid state stack ออก ใช้ซ้ำได้กับการ์ดสไตล์ HTR-1-028/AND-1-019)
- **engine hook รอบ CGH**: `p._playedApCostsThisTurn`, `p._eventsUsedThisTurn`, `unit.enteredTurn` (`G.turn` ตอนลงสนาม จากทุกช่องทาง), `tempUnblockableBP`/`tempUnblockableBPMin` (แจกชั่วคราว), `findSeg()` helper ใน common.js (ลอง matcher กับทุก '@'-segment — เชื่อม on-event เข้ากับ on-play matcher เดิม)
- **engine hook รอบ ARK**: `unit.skipNextStand`/`unit.noRetire`/`unit.tempSnipe`/`unit._playedByEffect` fields, `auraBp(owner,srcUnit,tgtUnit)` hook, `H.lookTopAndDiscard` (mill top-N ไป Outside Area ที่เหลือกลับไปบนเด็ค)
- **engine hook รอบ HTR**: `onBeforeLeaveCounters`, `unit._watchers` array, `onAnyWinBattle`/`onAnyLoseBattle`, `retireAtEndOfTurn`/`noBlock`/`_grantedOnWinDraw` unit fields, `_playedTraitsThisTurn`, `onBlock` ส่ง attacker unit เป็น param ที่ 4
...
| `js/effects/mcr.js`, `eva.js`, `htr.js`, `ark.js`, `cgh.js`, `and.js` | script รายใบ (registry per cardNo — override generic เสมอ) |
...
Effect hooks ที่มีให้ใช้ (ลง `Effects.registry[cardNo]`): `onPlay onAttack onBlock(atkUnit) onEvent onMain onLeaveField onSideline(reason) onTurnStart onRaided onColorTrigger onWinBattle onDefenderWinBattle(atkP,atkUnit) onAnyWinBattle onAnyLoseBattle onBeforeLeaveCounters(reason)` (async) + `bpBonus impactBonus genMod costMod frontGenBonus auraBp(owner,srcUnit,tgtUnit)` (sync, คำนวณสด — `auraBp` ห้ามเรียก `Engine.bp()` ซ้ำ จะ recursion)
Unit state: `bpMod` (หมดเทิร์น) `bpPersist` (ถึงต้นเทิร์นหน้า) `tempImpact tempDmg tempGen tempFrontGen frontGenPersist counters retireAtEndOfMain retireAtEndOfTurn noBlock skipNextStand noRetire tempSnipe tempUnblockableBP tempUnblockableBPMin effectsNullified enteredTurn _grantedOnWinDraw _playedByEffect _watchers[]`
